...
const DeleteTweetBtn = ({ id, imageUrl }: { id: number, imageUrl: string | null }) => {
    return (
        <Dropdown.Root>
            <Dropdown.Trigger className="outline-none" id="delete-menu">
            ...
            <Dropdown.Portal>
                <Dropdown.Content className="bg-white p-1 shadow-md rounded flex flex-col gap-1">
                    <Dropdown.Item id="delete-btn" onClick={() => deleteTweet(imageUrl, id)} className="text-xs font-medium cursor-pointer rounded text-red-600 px-2 py-1 outline-none hover:bg-red-50">
                        Delete Tweet
                    </Dropdown.Item>
